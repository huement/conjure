class Homestead
    def Homestead.configure(config, settings, wizard)
        # Set The VM Provider
        ...
        # Configure The Box
        config.vm.define wizard["name"] ||= "wizard-staff"
        config.vm.box = settings["box"] ||= "laravel/homestead"
        config.vm.box_version = settings["version"] ||= ">= 4.0.0"
        config.vm.hostname = wizard["hostname"] ||= "homestead"

        # Configure A Private Network IP
        if settings["ip"] != "autonetwork"
            config.vm.network :private_network, ip: settings["ip"] ||= "192.168.23.13"
        else
        ...
        config.vm.provider "virtualbox" do |vb|
            vb.name = wizard["name"] ||= "wizard-staff"
        ...
        ["vmware_fusion", "vmware_workstation"].each do |vmware|
            config.vm.provider vmware do |v|
                v.vmx["displayName"] = wizard["name"] ||= "wizard-staff"
        ...
        config.vm.provider "parallels" do |v|
            v.name = wizard["name"] ||= "wizard-staff"
