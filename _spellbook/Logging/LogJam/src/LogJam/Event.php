...
class Event
{
	...
	public function init_log( $logger_class )
	{
		if ( class_exists( $logger_class ) ) {
			...
			if ( is_a( $logger, 'LogJam\Logger' ) ) {
				$this->loggers[] = $logger;
				return $this->loggers;
			}
		}

		return new \WP_Error( 'Incorrect `LogJam\Logger` object' );
	}
	 ...
	 *
	 * @param Logger $logger          LogJam\Logger object.
	...
	public function watch( Logger $logger, $additional_args = array() )
	{
		...
		if ( ! is_a( $log, 'LogJam\Log' ) || ! $log->is_log() ) {
			return;
		}
		 ...
		 */
		$active_levels = apply_filters( 'logjam_active_levels', array(
			'fatal',
			'error',
			'warn',
			'info',
	...
	public function shutdown()
	{
		...
		do_action( 'logjam_after_save_log', $this->logs );
	}
	...
	private function save_log( Log $log_object )
	{
		...
		update_post_meta( $post_id, '_logjam_label', $log->meta['label'] );
		update_post_meta( $post_id, '_logjam_log_level', $log->meta['log_level'] );
		update_post_meta( $post_id, '_logjam_ip', $log->meta['ip'] );

		update_post_meta( $post_id, '_logjam', $log->meta );
	}
}
