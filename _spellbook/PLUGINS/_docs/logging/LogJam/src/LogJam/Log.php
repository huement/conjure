...
class Log
{
	...
	public static function get_the_log( $post )
	{
		...
		$log['label'] = get_post_meta( $post->ID, '_logjam_label', true );
		$log['level'] = get_post_meta( $post->ID, '_logjam_log_level', true );
		$log['meta'] = get_post_meta( $post->ID, '_logjam', true );
	...
	protected static function get_ip()
	{
		 ...
		 */
		return apply_filters( 'logjam_log_remote_ip', $ip );
	}
