...
abstract class Logger
{
	...
	public function __construct() {
		if ( empty( $this->label ) ) {
			wp_die( '`LogJam\Logger\Logger` requires the `$label` property.' );
		}
		if ( empty( $this->hooks ) || ! is_array( $this->hooks ) ) {
			wp_die( '`LogJam\Logger\Logger` requires the `$hooks` property.' );
		}
	}
	...
	/**
	 * Set `\LogJam\Log` to the `$this->log`.
	 *
	 * @param Log    $log             An instance of `LogJam\Log`.
	 */
	...
	/**
	 * Returns the value of `LogJam`.
	 *
	 * @return string Log level that come from `LogJam` class.
	 */
	...
	public function get_table( $array )
	{
		if ( ! empty( $array ) ) {
			...
			return '<table class="table-logjam">'
			       . implode( "", $cols ) . '</table>';
		}
	}
